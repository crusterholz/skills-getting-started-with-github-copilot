    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantList = details.participants.length
          ? `<ul class="participants-list">${details.participants.map(participant => `<li><span class="participant-name">${participant}</span><button class="participant-remove" data-activity="${name}" data-email="${participant}" aria-label="Remove ${participant}">×</button></li>`).join("")}</ul>`
